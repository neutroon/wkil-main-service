...
- `src/modules/meta/whatsapp/whatsappCoexistence.service.ts`
  - Added payload parsers, stable job-ID builders, retry/retention policy, and fail-loud worker dispatch hooks until the later importers are installed.
- `src/modules/meta/core/meta.queue.ts`
  - Added coexistence job types, per-job BullMQ option forwarding, diagnostic opt-out for webhook latency, dedicated queue envelopes, actual queue-type logging, and worker dispatch branches.
- `src/modules/meta/whatsapp/whatsapp.controller.ts`
  - Validates `history` and `smb_app_state_sync` webhook fields, queues one history job per chunk with bounded concurrency 4 and one bounded contacts job, skips empty sync arrays as no-ops, acknowledges valid events, and keeps live-message routing unchanged.
...
- Diagnostic opt-out and option forwarding: failed because diagnostics still ran for coexistence jobs, then passed after the internal skip option was added; the same test verifies BullMQ receives `jobId`, attempts, backoff, and retention options while existing jobs retain diagnostics.
- Missing-importer behavior: failed because valid hooks resolved successfully, then passed after hooks began throwing explicit importer-not-installed errors.
- Empty sync handling and bounded history concurrency: failed on the prior 400/sequential behavior, then passed after empty arrays became no-ops and history enqueueing became batch-concurrent with a maximum of four in flight.

Final relevant Vitest results:

- `npm test -- src/modules/meta/whatsapp/whatsappCoexistence.service.test.ts`: **7/7 tests passed**.
- `npm test -- src/modules/meta/whatsapp/whatsapp.controller.test.ts`: **8/8 tests passed**.
- `npm test -- src/modules/meta/core/meta.queue.test.ts`: **1/1 test passed**.
- Combined final relevant run across those three files: **16/16 tests passed**.
...
- The controller only validates, queues, logs safe counts/identifiers, and acknowledges; it does not look up accounts, import synchronously, or enqueue live-message jobs for sync events. History chunks are enqueued in bounded concurrent batches, and post-enqueue queue diagnostics are skipped only for these webhook jobs.
- Worker dispatch is dynamic and isolated from the live Meta message processor. Until the later history/contact importer tasks replace the hooks, valid jobs fail explicitly so BullMQ retries/retains them rather than treating them as successful imports.

## Unresolved concerns

- The dispatch hooks intentionally fail until Tasks 2 and 3 wire the persistence importers; deploying Task 1 alone will leave sync jobs retrying/failed rather than importing data, so the importer tasks must be deployed before the retry budget is exhausted.
...
- The contract caps contacts at 1,000 entries; if Meta emits valid oversized batches, the parser contract will need a narrowly scoped adjustment.
