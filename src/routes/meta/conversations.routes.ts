...
import { toggleAiSchema, updateStatusSchema, sendMessageSchema } from "../../validations/conversation.validation";
 ...
 *   }
 */
conversationsRoutes.get("/", authenticateToken, validate(paginationSchema), async (req: Request, res: Response) => {
...
conversationsRoutes.get("/:id/messages", authenticateToken, validate(idPaginationSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
 ...
 *   { success: true }
 */
conversationsRoutes.patch("/:id/read", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
 ...
 */
conversationsRoutes.post("/:id/typing", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
  ...
  if (conversation.channel === "messenger") {
    ...
    if (page) {
      const token = decryptFacebookSecret(page.pageAccessToken);
      await sendMessengerAction(conversation.senderId, typing ? "typing_on" : "typing_off", token).catch((e) =>
 ...
 */
conversationsRoutes.patch("/:id/ai-toggle", authenticateToken, validate(idParamSchema), validate(toggleAiSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
 ...
 */
conversationsRoutes.patch("/:id/status", authenticateToken, validate(idParamSchema), validate(updateStatusSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
 ...
 */
conversationsRoutes.delete("/:id", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
...
conversationsRoutes.post("/:id/messages", authenticateToken, validate(idParamSchema), validate(sendMessageSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
  ...
  const { sendWhatsAppReply } = require("../../services/meta/whatsapp.service");
...
conversationsRoutes.get("/templates", authenticateToken, async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
...
conversationsRoutes.post("/:id/template", authenticateToken, validate(idParamSchema), validate(sendWhatsAppTemplateSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
...
// ============================================================================
conversationsRoutes.put("/:id/messages/:mid/approve", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
  const { id: conversationId } = req.params as any;
...
conversationsRoutes.delete("/:id/messages/:mid", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
  const messageId = parseInt(req.params.mid, 10);
...
conversationsRoutes.post("/:id/suggest", authenticateToken, validate(idParamSchema), async (req: Request, res: Response) => {
  const userId = (req as any).user.id as number;
  const { id: conversationId } = req.params as any;
